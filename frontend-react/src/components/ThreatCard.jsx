...
function ThreatCard({ title = 'Threat Result', item, compact = false }) {
  ...
  return (
    <Motion.article
      ...
      className="glass-card scan-lines relative overflow-hidden rounded-3xl p-5"
    >
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.35em] text-[#00CFFF]">{title}</p>
        ...
        {timestamp && <p className="text-sm text-slate-400">Timestamp: {new Date(timestamp).toLocaleString()}</p>}
        ...
        {!compact && explanation.length > 0 && (
          <div className="rounded-2xl border border-[#00FF9F]/12 bg-[#00FF9F]/6 p-4">
